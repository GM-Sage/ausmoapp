...
export const communicationApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    createButton: builder.mutation<CommunicationButton, Partial<CommunicationButton>>({
      query: (button) => ({
        url: 'buttons',
        method: 'POST',
        body: button,
    ...
    updateButton: builder.mutation<CommunicationButton, CommunicationButton>({
      ...
      invalidatesTags: (result, error, button) => [{ type: 'Button', id: button.id }],
    }),
    ...
    // Messages
    getMessages: builder.query<PaginatedResponse<Message>, { userId: string; page?: number; limit?: number }>({
    ...
    updateMessage: builder.mutation<Message, Message>({
      ...
      invalidatesTags: (result, error, message) => [{ type: 'Message', id: message.id }],
    }),
