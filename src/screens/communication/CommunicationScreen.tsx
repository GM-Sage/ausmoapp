...
import { navigateToPage, navigateBack, navigateToHome } from '../../store/slices/navigationSlice';
...
export default function CommunicationScreen() {
  ...
  const currentPage = useSelector((state: RootState) => state.navigation.currentPageId);
  const currentBook = useSelector((state: RootState) => state.navigation.currentBookId);
  ...
  const loadCurrentPage = async () => {
    try {
      ...
      if (!currentPage || !currentBook || !currentUser) {
        console.log('Missing required data:', { currentPage, currentBook, currentUser: !!currentUser });
      ...
      const pageData = await SupabaseDatabaseService.getPage(currentPage);
  ...
  const loadFirstAvailablePage = async () => {
    try {
      ...
      const books = await SupabaseDatabaseService.getBooksByUser(currentUser.id);
  ...
  const toggleSwitchScanning = () => {
    ...
    if (isSwitchScanning) {
      ...
      scanService.startScanning(totalRows, totalColumns, gridSize, 'row-column');
      setIsSwitchScanning(true);
    }
  };
    ...
    try {
      ...
      if (button.action.type === 'speak') {
        ...
        if (message) {
          ...
          console.log('Calling AudioService.speak with:', { message, voiceSettings });
        ...
        if (button.action.targetPageId) {
          // Track navigation
          analyticsService.trackNavigation(page?.id, button.action.targetPageId, currentBook || '');
          ...
          <View style={[styles.container, { backgroundColor: page.backgroundColor }]}>
            ...
            <View style={styles.header}>
              ...
              <View style={styles.headerButtons}>
                <TouchableOpacity 
                  style={[styles.headerButton, isSwitchScanning && styles.headerButtonActive]} 
                  onPress={toggleSwitchScanning}
                  accessible={true}
                  accessibilityLabel={isSwitchScanning ? "Stop switch scanning" : "Start switch scanning"}
                  accessibilityRole="button"
                >
                  <Ionicons 
                    name={isSwitchScanning ? "stop-circle" : "play-circle"} 
                    size={24} 
                    color={isSwitchScanning ? COLORS.ERROR : COLORS.SURFACE} 
                  />
                </TouchableOpacity>
                <TouchableOpacity 
                  ...
                  accessibilityRole="button"
                >
                  <Ionicons name="volume-high" size={24} color={COLORS.SURFACE} />
                </TouchableOpacity>
              </View>
            </View>
