...
import AnalyticsService, { ProgressReport, ChartData } from '../../services/analyticsService';
...
export default function AnalyticsScreen() {
  ...
  const [selectedChart, setSelectedChart] = useState<'usage' | 'vocabulary' | 'efficiency'>('usage');
  ...
  const loadProgressReport = async () => {
    ...
    try {
      setIsLoading(true);
      const report = await analyticsService.generateProgressReport(currentUser.id, selectedPeriod);
  ...
  const handleExportReport = () => {
    Alert.alert('Export Report', 'Export functionality coming soon');
  };
        ...
        {periods.map((period) => (
          ...
          >
            <Ionicons 
              ...
              size={20} 
              color={selectedPeriod === period.key ? COLORS.SURFACE : COLORS.PRIMARY} 
            />
            <Text style={[
        ...
        {cards.map((card, index) => (
          <View key={index} style={styles.summaryCard}>
            <View style={[styles.cardIcon, { backgroundColor: card.color }]}>
              <Ionicons name={card.icon as any} size={24} color={COLORS.SURFACE} />
            </View>
  ...
  const renderAchievements = () => {
    if (!progressReport || progressReport.achievements.length === 0) return null;
  ...
  const renderRecommendations = () => {
    if (!progressReport || progressReport.recommendations.length === 0) return null;
  ...
  const renderImprovementAreas = () => {
    if (!progressReport || progressReport.improvementAreas.length === 0) return null;
          ...
          {charts.map((chart) => (
            ...
            >
              <Ionicons 
                ...
                size={16} 
                color={selectedChart === chart.key ? COLORS.SURFACE : COLORS.PRIMARY} 
              />
              <Text style={[
